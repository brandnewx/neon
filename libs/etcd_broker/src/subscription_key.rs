...
//!
//! * [`SubscriptionFullKey`] full key format: `<cluster_prefix>/<node_kind>/<operation>/<tenant>/<timeline>/<node_id>`
...
pub enum SubscriptionKind {
    ...
    All,
    /// Get etcd updates, specific to a certain node kind.
    Node(NodeKind),
    /// Get etcd updates for a certain operation on specific nodes.
    Operation(NodeKind, OperationKind),
    /// Get etcd updates for any timeiline of a certain tenant, affected by a given operation, running on the node kind specified.
    OperationOnTenantTimelines(NodeKind, OperationKind, ZTenantId),
    /// Get etcd updates for a certain timeline of a tenant, affected by a given operation, running on the node kind specified.
    OperationOnTimeline(NodeKind, OperationKind, ZTenantTimelineId),
}
...
impl SubscriptionKey {
    ...
    pub fn operation(
        ...
        Self {
            cluster_prefix,
            kind: SubscriptionKind::Operation(node_kind, operation),
        }
    }
    ...
    pub fn sk_timeline_info(cluster_prefix: String, timeline: ZTenantTimelineId) -> Self {
        Self {
            cluster_prefix,
            kind: SubscriptionKind::OperationOnTimeline(
                NodeKind::Safekeeper,
    ...
    pub fn watch_key(&self) -> String {
        ...
        match self.kind {
            ...
            SubscriptionKind::Node(node_kind) => {
                format!("{cluster_prefix}/{node_kind}")
            }
            SubscriptionKind::Operation(node_kind, operation_kind) => {
                format!("{cluster_prefix}/{node_kind}/{operation_kind}")
            }
            SubscriptionKind::OperationOnTenantTimelines(node_kind, operation_kind, tenant_id) => {
                format!("{cluster_prefix}/{node_kind}/{operation_kind}/{tenant_id}")
            }
            SubscriptionKind::OperationOnTimeline(
                node_kind,
                operation_kind,
                ZTenantTimelineId {
                    tenant_id,
            ...
            ) => format!("{cluster_prefix}/{node_kind}/{operation_kind}/{tenant_id}/{timeline_id}"),
        }
    }
}
...
impl Display for SubscriptionFullKey {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(f, "{node_kind}/{operation}/{id}/{node_id}")
    }
}
...
impl FromStr for SubscriptionFullKey {
    ...
    fn from_str(subscription_kind_str: &str) -> Result<Self, Self::Err> {
        ...
        Ok(Self {
            node_kind: parse_capture(&key_captures, 1)?,
            operation: parse_capture(&key_captures, 2)?,
            id: ZTenantTimelineId::new(
                parse_capture(&key_captures, 3)?,
                parse_capture(&key_captures, 4)?,
            ),
...
mod tests {
    ...
    fn full_cluster_key_parsing() {
        ...
        let timeline_subscription_keys = [
            ...
            SubscriptionKey {
                cluster_prefix: prefix.to_string(),
                kind: SubscriptionKind::Node(node_kind),
            },
            SubscriptionKey {
                cluster_prefix: prefix.to_string(),
                kind: SubscriptionKind::Operation(node_kind, operation_kind),
            },
            SubscriptionKey {
                cluster_prefix: prefix.to_string(),
                kind: SubscriptionKind::OperationOnTenantTimelines(
                    node_kind,
                    operation_kind,
                    tenant_id,
                ),
